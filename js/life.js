const life = (() => {
  ...
  const newBoard = (shouldSeed) => {
    ...
    if (shouldSeed) {
      board = board.map((row) =>
        row.map((_) => (getRandomInt(100) < THRESHOLD ? LIVE : DEAD))
      );
    }
    return board;
  };
  ...
  const neighborCoordinatesForRow = (r, x = 0) =>
    coordsForRow(r, x).map((xyArr) => neighborCoordinates(...xyArr));
  ...
  const boardAsNumberOfNeighbors = (board, arrayOfNeighborCoords) => {
    return neighborCellsForCoordinateArray(board, arrayOfNeighborCoords).map(
      (neighborCellsForRow) => {
  ...
  const numberRowAsLiveDeadCells = (rowOfNumbers, rowOfCells) =>
    rowOfNumbers.map((n, i) => numberToLiveDead(n, rowOfCells[i]));
